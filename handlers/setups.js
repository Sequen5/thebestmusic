...
module.exports = (client) => {
    ...
    client.on("ready", () => {
        setInterval(() => {
            ...
            if (stateswitch) client.user.setActivity(`${config.prefix}help | The Best Music`, {
                type: "PLAYING"
            });
            else client.user.setActivity(` ${client.guilds.cache.size}k Server`, {
                type: "PLAYING"
            });
        }, 5000); //5 second delay
    ...
    function getAll(client, guild) {
        const embed = new MessageEmbed()
            ...
            .addField("**__Music - Supported sources__**", `
                >>> \`Youtube\`, \`Soundcloud\`
                `)
            .setFooter("To see command descriptions and usage type   s!help [CMD Name]", client.user.displayAvatarURL())
